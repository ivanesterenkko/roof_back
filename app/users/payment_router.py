...
from .schemas import CompanyProjectResponse, OrderRequest, SUserRegister, UserResponse
...
async def create_order(
    ...
    new_order = await OrdersDAO.add(
        ...
        is_paid=False,
        price_rate=tariff.price*order_data.duration,
        uuid_order=data_payment["Data"]["operationId"]
    )
    print(new_order)

    if data_payment:
        return {
            "payment_url": data_payment["Data"]["paymentLink"]
        }
    else:
        return None
...
@version(2)
async def successful_payment_card(
        operation_id: str
):

    order: Orders = await OrdersDAO.find_by_id(uuid_order=operation_id)
